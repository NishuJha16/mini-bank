...
function myFn4(){
  ...
  icon: "error",
  title: 'Oops...',
  text: 'Receiver name or amount cannot be empty!!'
}).then(function()
{
  window.history.back();
});
   ...
   mysqli_select_db($conn,'login');
    ...
    if(isset($_POST['submit'])){
           ...
           if(empty( $_POST['sender']) && empty( $_POST['receiver']) && empty( $_POST['amount'])){
                 ...
                 $receiverQuery="SELECT * FROM users WHERE id='". $receiver ."'";
                 ...
                 while($senderRow = mysqli_fetch_array($senderResult))
                 {
                  while($receiverRow=mysqli_fetch_array($receiverResult))
                  {
                    ...
                    {
                      $insertQuery="INSERT INTO history (sender,receiver,amount,date,status) VALUES ('". $senderName ."','". $receiverName ."','". $amount ."',now(),'failed')";
                    ...
                    $deductQuery="UPDATE users SET balance='". $newSenderAmt ."' WHERE id='". $sender ."'";
                    $addQuery="UPDATE users SET balance='". $newReceiverAmt ."' WHERE id='". $receiver ."'";
                    $insertQuery="INSERT INTO history (sender,receiver,amount,date,status) VALUES ('". $senderName ."','". $receiverName ."','". $amount ."',now(),'success')";
